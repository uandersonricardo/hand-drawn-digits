  ...
  Atrament.prototype.scaleDown = function () {
   ...
   // var array = pixels.reduce(function (arr, row) {return [].concat(arr, row.reduce(function (concatedRow, alpha) {return [].concat(concatedRow, [0, 0, 0, alpha * 255]);}, []));}, []);
    ...
    for(var i = 0; i < 28; i++){
      matrix[i] = [];
      for(var j = 0; j < 28; j++){
        var imageData = mnistCtx.getImageData(j, i, 1, 1);
    ...
    // Dispara o evento.
    mnistCanvas.dispatchEvent(new CustomEvent('draw', { 'detail': matrix }));
  };
...
var sketcher = new Atrament('#original', 400, 400);